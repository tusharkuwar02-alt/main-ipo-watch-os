...
type SmartStock = {
  ...
  hedgeState: string; confirmation: string; institutional?: InstitutionalEvidence;
};
...
export default function SmartMoneyDashboard({ initialData }: { initialData: SmartScan }) {
  ...
  const stocks = useMemo(() => initialData.stocks.filter(stock =>
    (!onlyShortlist || stock.actionable) &&
  ...
  ), [initialData.stocks, query, direction, confirmation, onlyShortlist]);
  ...
  return <main className="smartMoney">
    ...
    <section className="rulebar smartRules"><span>✓ Actual NSE MF/FPI filings</span><span>✓ Point-in-time availability</span><span>✓ Client-netted prior large deals</span><span>✓ Daily FII/DII excluded</span><span>✓ Score is ranking—not entry</span><span>✓ 2R/3R levels are reference only</span></section>
    ...
    <footer>Institutional Score ≥3 = stronger context only · No BUY/SELL alert · Verify live prices before trading</footer>
  </main>;
}
...
function SmartRow({ stock, open, toggle }: { stock: SmartStock; open: boolean; toggle: () => void }) {
  ...
  return <>
    <tr className="stockRow">
      ...
      <td data-label="Strategy Reference">{stock.setup === "Avoid Long" ? <><b className="down">No long reference</b><span>Wait for price reversal</span></> : <><b>Trigger ref ₹{stock.entry.toFixed(2)}</b><span>SL ref ₹{stock.stopLoss.toFixed(2)}</span><small>2R ₹{stock.target1.toFixed(2)} · 3R ₹{stock.target2.toFixed(2)}</small></>}</td>
