...
async function cachedJson(kind,key,url,optional=false){
  ...
  for(let attempt=0;attempt<4;attempt++){try{const response=await fetch(url,{headers:nseHeaders,signal:AbortSignal.timeout(45000)});if(!response.ok)throw new Error(`${response.status}`);const json=await response.json();await fs.writeFile(file,JSON.stringify(json));return json;}catch(error){if(attempt===3){if(optional){apiGaps.push(`${kind}:${key}`);console.warn(`optional source gap ${kind} ${key}: ${error.message}`);return [];}throw new Error(`${kind} ${key}: ${error.message}`);}await new Promise(resolve=>setTimeout(resolve,1500*(attempt+1)));}}
}
...
async function bulkDeals(){
  const months=monthsBetween("2021-10-01",END),requests=months.flatMap(date=>["bulk_deals","block_deals"].map(type=>({date,type}))),pages=await mapLimit(requests,2,async({date,type})=>{const {first,last}=apiRange(date);return cachedJson(type,monthKey(date),`https://www.nseindia.com/api/historicalOR/bulk-block-short-deals?optionType=${type}&from=${first}&to=${last}`,true);});
...
async function main(){
  ...
  const holdings=await holdingsBySymbol(baseSignals),deals=await bulkDeals();
