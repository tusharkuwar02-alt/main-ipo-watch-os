...
export function weeklyBreakoutStatus(asOf, currentWeekStart) {
  const asOfWeekStart = startOfWeek(asOf.toISOString().slice(0, 10));
  const day = asOf.getUTCDay();
  return currentWeekStart === asOfWeekStart && day >= 1 && day <= 4
    ? "week-in-progress"
    : "confirmed";
}
...
export function evaluateStock(stock, bars, niftyBars, asOf = new Date()) {
  ...
  if (weekly.length >= 52) {
    ...
    const weeklyStatus = weeklyBreakoutStatus(asOf, currentWeek.date);
