...
export const SYSTEM_FAMILY = {
  "daily-sma21": "SMA Proximity", "daily-sma30": "SMA Proximity", "weekly-sma21": "SMA Proximity", "weekly-sma30": "SMA Proximity",
  "daily-breakout": "Breakout", "weekly-51": "Breakout", ath: "Breakout", "listing-high": "Breakout", "high-5d": "Breakout", "high-10d": "Breakout", "high-20d": "Breakout",
  "ipo-base": "IPO Structure", "issue-price": "IPO Structure",
  "volume-expansion": "Volume", "volume-dry-up": "Volume", "pocket-pivot": "Volume",
  "rs-nifty": "Relative Strength", "rs-new-high": "Relative Strength",
  "tight-close": "Price Pattern", "gap-up-hold": "Price Pattern"
};
...
export function marketDateInIndia(asOf) {
  return new Date(asOf.getTime() + 330 * 60 * 1000).toISOString().slice(0, 10);
}

export function expectedWeekClose(currentWeekStart, holidays = []) {
...
export function weeklyBreakoutStatus(asOf, currentWeekStart, marketDate = "", holidays = []) {
  const asOfDate = marketDateInIndia(asOf);
  const asOfWeekStart = startOfWeek(asOfDate);
  if (currentWeekStart < asOfWeekStart) return "confirmed";
  const expectedClose = expectedWeekClose(currentWeekStart, holidays);
  return asOfDate >= expectedClose && marketDate >= expectedClose ? "confirmed" : "week-in-progress";
}

export function scoreConfluence(matches) {
  const families = [...new Set(matches.map(match => match.family || SYSTEM_FAMILY[match.id] || "Other"))];
  return {
    signalFamilies: families,
    confluenceScore: Math.min(100, families.length * 20 + Math.max(0, matches.length - families.length) * 2)
  };
}
...
function add(matches, id, reason, extra = {}) {
  matches.push({ id, name: SYSTEM_NAME[id], family: SYSTEM_FAMILY[id], reason, ...extra });
}

export function evaluateStock(stock, bars, niftyBars, asOf = new Date(), context = {}) {
  if (!bars.length) return { matches: [], diagnostics: { error: "no-history" } };
  ...
  const avgVol20 = bars.length >= 21 ? avg(volumes.slice(-21, -1)) : NaN;
  const previous20High = bars.length >= 21 ? max(bars.slice(-21, -1).map(b => b.high)) : NaN;
  if (Number.isFinite(previous20High) && latest.close > previous20High && latest.volume >= avgVol20 * 1.25)
  ...
  if (weekly.length >= 52) {
    ...
    const weeklyStatus = weeklyBreakoutStatus(asOf, currentWeek.date, context.marketDate || latest.date, context.holidays || []);
    if (currentWeek.close > prior51) add(matches, "weekly-51", weeklyStatus === "week-in-progress" ? "51-week breakout — week in progress" : "51-week breakout — confirmed completed week", { weeklyStatus });
  }
  ...
  if (stock.issuePrice > 0 && latest.close >= stock.issuePrice && (prior?.close < stock.issuePrice || bars.slice(-Math.min(3, bars.length)).every(b => b.close >= stock.issuePrice)))
    add(matches, "issue-price", `Issue price ₹${stock.issuePrice.toFixed(2)} reclaimed/held`);
  if (prior && Number.isFinite(avgVol20) && latest.close > prior.close && latest.volume >= avgVol20 * 1.5)
  ...
  if (Number.isFinite(dryDistance) && Number.isFinite(avgVol20) && Math.abs(dryDistance) <= 0.02 && latest.volume <= avgVol20 * 0.6)
  ...
  if (prior && latest.open >= prior.close * 1.03 && latest.low >= prior.close * 1.01 && latest.close >= latest.open)
    add(matches, "gap-up-hold", `Gap ${(latest.open / prior.close * 100 - 100).toFixed(2)}% held above prior close`);

  return {
    matches,
    diagnostics: {
      dailySma21: Number.isFinite(dailySma21) ? +dailySma21.toFixed(2) : null,
      dailySma30: Number.isFinite(dailySma30) ? +dailySma30.toFixed(2) : null,
      close: latest.close, changePct: prior ? +((latest.close / prior.close - 1) * 100).toFixed(2) : 0,
      volumeRatio: Number.isFinite(avgVol20) ? +(latest.volume / avgVol20).toFixed(2) : null
    }
  };
}
