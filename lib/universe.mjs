...
function parseCalendarDate(value) {
  ...
  const monthNumber = name => MONTHS[name.toLowerCase()] || Object.entries(MONTHS).find(([month]) => month.startsWith(name.toLowerCase()))?.[1];
  if (monthFirst && monthNumber(monthFirst[1])) return { year: Number(monthFirst[3]), month: monthNumber(monthFirst[1]), day: Number(monthFirst[2]) };
  if (dayFirst && monthNumber(dayFirst[2])) return { year: Number(dayFirst[3]), month: monthNumber(dayFirst[2]), day: Number(dayFirst[1]) };
  return null;
}
...
export function parseHdfcPastIpo(html) {
  const $ = cheerio.load(html);
  const records = [];
  $("table").each((_, table) => {
    const headers = $(table).find("thead tr").first().find("th,td").map((__, cell) => $(cell).text().replace(/\s+/g, " ").trim()).get();
    const nameIndex = headers.findIndex(header => /^Name$/i.test(header));
    const listingDateIndex = headers.findIndex(header => /^Date of Listing$/i.test(header));
    const issuePriceIndex = headers.findIndex(header => /^Issue Price$/i.test(header));
    if (nameIndex < 0 || listingDateIndex < 0 || issuePriceIndex < 0) return;
    $(table).find("tbody tr").each((__, row) => {
      const cells = $(row).find("td");
      const company = cells.eq(nameIndex).text().replace(/\s+/g, " ").trim();
      const parsed = parseCalendarDate(cells.eq(listingDateIndex).text());
      if (!company || !parsed) return;
      records.push({
        company,
        listingDate: `${parsed.year}-${String(parsed.month).padStart(2, "0")}-${String(parsed.day).padStart(2, "0")}`,
        issuePrice: parseMoney(cells.eq(issuePriceIndex).text()),
        year: parsed.year,
        source: "hdfc-past-ipo"
      });
    });
  });
  return records;
...
export function buildRollingUniverse(nseRows, performanceRows, listingRows, now = new Date()) {
  ...
  for (const ipo of performanceRows) {
    ...
    if (!ranked[0] || ranked[0].score < 0.54) {
      const sameDateRows = ipo.listingDate ? nseRows.filter(row => row.listingDate === ipo.listingDate) : nseRows;
      const expired = sameDateRows.map(row => ({ row, score: row.symbol === aliasSymbol ? 1 : similarity(ipo.company, row.company) })).sort((a, b) => b.score - a.score)[0];
      // Secondary IPO feeds contain both mainboard and SME issues. NSE series is the
      // authoritative segment gate, so a validated SM/ST match is an exclusion, not a failure.
      if (expired?.score >= 0.54 && ["SM", "ST"].includes(expired.row.series)) continue;
      if (expired?.score >= 0.54 && expired.row.listingDate < cutoffIso) continue;
      // This cross-exchange fallback also contains BSE-only issues. Absence from the
      // NSE master means it is outside this NSE-only product, not a broken NSE match.
      if (ipo.source === "hdfc-past-ipo") continue;
